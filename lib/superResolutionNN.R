...
superResolutionNN <- function(LR_dir, HR_dir, modelList, index){
  ...
  for(k in 1:n_files){
    ...
    predMat <- testNN(modelList, featMat)
    ...
    display(img_out)
    writeImage(img_out, paste0("../data/train/testHRNN/",  "img", "_", sprintf("%04d", i), ".jpg"))
