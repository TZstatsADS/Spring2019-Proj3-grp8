...
superResolution_lgb <- function(LR_dir, HR_dir, modelList,index){
  ...
  for(k in 1:n_files){
    ...
    predMat <- test.lgb(modelList, featMat)
  ...
  testlgbmse<-mean(mse)
  testlgbpsnr<- -10*log10(testlgbmse)
  return(c(testlgbmse,testlgbpsnr))
